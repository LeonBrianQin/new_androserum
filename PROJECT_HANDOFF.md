...
- `data/reports/` 现有 baseline vs finetuned 的同口径对比：
  - `baseline_vs_finetuned_dev200_5k.{png,json}`
  - `baseline_vs_finetuned_dev200_10k.{png,json}`
  - `baseline_vs_finetuned_dev200_50k.{png,json}`
- 当前最强的 `50k` 对比结果（以 SuSi 弱标签为代理）：
  - baseline：`NMI = 0.1301`，`ARI = -0.0151`，`silhouette_non_noise = 0.3233`
  - finetuned（A+B）：`NMI = 0.6062`，`ARI = 0.2626`，`silhouette_non_noise = 0.6264`
  - 结论：A+B finetuning 明显提升了 embedding 空间与功能弱标签的一致性，说明“功能相近的方法更接近”这一目标得到了支持
...
- ✅ baseline vs finetuned 的 `5k / 10k / 50k` 同口径对比已完成，A+B 可视为当前有效 baseline
- ✅ 信号 `E`（同 override）所需的数据抽取与训练接线已完成，本地 smoke test / 小规模干跑已通过
- 当前下一工程块：**在 AutoDL 上复跑 200 样本的 A+B+E 全量训练**
- Phase 5–7：未启动。
...
➡️ **用现有 `data/overrides/` 和 `data/methods/`，在 AutoDL 上复跑 200 样本的 A+B+E 全量训练。**  
➡️ **训练结束后，继续做 `A+B` vs `A+B+E` 的同口径聚类代理指标 / 图形对比。**
