...
4. **当前状态在第 5 节**：原项目（`/usr/local/python_projects/DexBERT/`）里已经写好的工具脚本和 asset 都列清楚了。
...
│  embeddings/baseline/<sha>.npy (云 GPU, ~30 分钟)                  │
...
### 任务 7.5（最终）：注释掉 `tests/test_*.py` 里跑通的部分以后，进入 Phase 3 → Phase 4

按流水线顺序推进。每个 Phase 一个 entrypoint 脚本（`scripts/0X_xxx.py`），按数字顺序跑。
...
- `README.md` 里 `.pt` 大小写的 "~330 MB"，实际 1.8 GB，待修
- 5 个空目录（`docs/`、`notebooks/`、`scripts/`、`third_party/`、`tests/` 之外的）GitHub 上不显示（git 不追踪空目录）；写 Phase 2 第一个真实文件时自然激活

**下一步起点（任何 agent 续上时从这里开始）**：

➡️ **Phase 2：Method 切分 + 过滤 + SuSi 打标**（详见 §7.4）

| # | 子任务 | 产物 | 估时 |
|---|--------|------|------|
| 1 | 测试数据准备：`ln -s /usr/local/python_projects/DexBERT/processed/0D64BB3C121E1986766505E182F16FB8DCC4188224F3094F99B9F905873DDC4A.txt data/processed/` | `data/processed/0D64....txt` 软链 | 5 min |
| 2 | 设计 schema：`src/androserum/data/schema.py` 定义 `Method` 数据类（11 字段：`apk_sha`、`class_name`、`method_sig`、`full_id`、`instructions`、`n_instr`、`api_calls`、`susi_cats`、`susi_dominant_cat`、`filtered`、可选 `bytecode_hash`）| `schema.py` | 15 min |
| 3 | 写 `method_extractor.py`：状态机解析 `.txt`（`ClassName:` / `MethodName:` / 空行作分隔），输出 `list[Method]` | `method_extractor.py` 核心 | 1 h |
| 4 | 过滤规则：`n_instr<5` / class 含 `R$` 或 `BuildConfig` / method 名以 `access$` / `lambda$` / `$values` / `<clinit>` 开头的 → `filtered=True` | （继续上一步）| 30 min |
| 5 | API 抽取：用正则 `invoke-(?:virtual|direct|static|interface|super)(?:/range)?\s+\{[^}]*\},\s+(L\S+;->\S+)` 抽出 `api_calls` 列表 | （继续）| 30 min |
| 6 | SuSi 类别表：从 <https://github.com/secure-software-engineering/SuSi> 拿 `sources_and_sinks.txt`，转成 `third_party/susi_categories.json`（`{api_path: category}` 哈希表）| `susi_categories.json` | 30 min |
| 7 | 写 `susi_tagger.py`：给每个 method 的 `api_calls` 查表，输出 `susi_cats`（命中类别集合）+ `susi_dominant_cat`（最频繁类别）| `susi_tagger.py` | 30 min |
| 8 | 端到端串：`scripts/02_extract_methods.py` 入口 + `tests/test_method_extractor.py` + `tests/test_susi_tagger.py`，在那 1 个测试 .txt 上跑出 `data/methods/0D64....parquet`，肉眼检查几行结果合理 | `.parquet` + 2 个 test | 1 h |

**关键工程注意**（§8.2）：`full_id` 字段必须是 `Lcom/foo/Bar;-><init>(II)V` 这种 Androguard 风格——以后 Phase 5 用 Androguard 抽 FCG 时要拿这个 `full_id` 跟 FCG 节点 id 直接对齐，**所以 method_extractor 里要写一个 `normalize_class_name` 函数把 DexBERT 反编译产物的 `com/foo/Bar` 形式 + `<init>(...)V` 形式拼成 Androguard 风格**。这是 Phase 5 最容易踩的坑，handoff §8.2 明确警告过。

预计 Phase 2 总工作量：**半天**。完成后再次更新本工作日志（追加一条 dated entry）。

---

**文档版本**：v1.1
**最后更新**：2026-05-06（会话 1 结束，骨架 + encoder 迁移 + 工程保险完成；新增 §11 工作日志）
**下次更新触发条件**：每次会话结束 / 完成 Phase 2 / 完成 Phase 4 / 重大决策变更
