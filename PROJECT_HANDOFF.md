...
- 当前关键 commits（自旧到新）：
  ...
  - `ded4d78` — Phase 5：FCG 抽取 / method-id 对齐 scaffold（本地已写好，等待首跑）

**已实现流水线（Phase 0 → 5，Phase 5 代码未执行）**：
...
- ✅ Phase 5 FCG 抽取 / method-id 对齐代码 scaffold 已写好，`run_all.py` 已接入 `do_fcg`
- 当前推荐主线：
  - **Phase 4 内容 encoder：A+B+E**
  - **下一工程块：Phase 5 首次全量执行（dev200）并验收对齐质量**
- Phase 6–7：尚未启动。
...
**2026-05-09**：仓库 `main` 已含 Phase 0–5 的代码骨架，其中 `Phase 4` 的 `AB / ABE / ABCE` 实验与同口径代理评估已完成，`Phase 5` 的抽取 / 对齐代码已写好但尚未执行（见 §5「新项目 new_androserum」与 §11 最新一条）。本节 **§7.1–7.4** 仍是早期历史步骤，仅作溯源；**接手开发请以 §11 最新一条为起点，当前实际下一步是执行 Phase 5 并验收 `full_id` 对齐质量。**
...
**文档版本**：v1.4
**最后更新**：2026-05-09（会话 6：ABCE 验收 + Phase 5 scaffold）
**下次更新触发条件**：Phase 5 首跑通 / Phase 6 设计落地 / 重大决策变更
