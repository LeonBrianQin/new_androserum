...
### 2026-05-14（会话 13）— lamda240 本地全预处理完成 + 云端 train-only 路线落地 + 5090 利用率基线

**本轮目标**：

- 把 `lamda240` 从“远端一条龙下载+训练”改为“**本地 CPU 预处理 / 云端 GPU 只训练**”
- 避免云端下载 AndroZoo 的长尾不稳定与代理/网络问题
- 为后续更大规模训练记录一组 `RTX 5090` 的初始吞吐基线，便于下一轮更 aggressive 调参

#### 1. 已确认的 `lamda240` 数据状态

- 候选与划分文件已在仓库中固定：
  - `configs/lamda_binary_eval_240_candidates.csv`
  - `configs/lamda_train_160.csv`
  - `configs/lamda_val_40.csv`
  - `configs/lamda_test_40.csv`
  - `configs/lamda_train_val_test.summary.json`
- 真实工作流不再依赖 `data_lamda_eval_dryrun/`
  - `dryrun` 仍保留作小样本回归集，不建议删除
  - 正式大集目录统一为 `data_lamda_eval/`

#### 2. 本地预处理（CPU）已完成

通过本地完整预处理，`lamda240` 已在 `data_lamda_eval/` 下完成：
...
- `Phase 2`：method 抽取
- `Phase 2b`：SuSi 打标
- `Phase 2c`：override sidecar
- `Phase 5`：FCG 抽取与对齐

**当前推荐工程分工正式调整为**：

- **本地**：`Phase 0 / 1 / 2 / 2b / 2c / 5`
- **云端**：`Phase 4 / 4 export / Phase 6 / 6 export`

这比“远端一条龙跑全流程”更稳，也更符合“服务器只做 GPU 任务”的原则。

#### 3. 新增的 lamda 专用脚本（当前有效入口）

**本地侧**：

- `scripts/13_download_lamda240_local.sh`
  - 从 `configs/lamda_binary_eval_240_candidates.csv` 生成 SHA 列表并在本地下载 APK
- `scripts/14_prepare_lamda240_local.sh`
  - 本地从下载开始一路跑到 `FCG`
- `scripts/14b_prepare_lamda240_local_no_download.sh`
  - 假设 APK 已准备好，直接从反编译开始跑到 `FCG`

**Windows 兜底下载侧**：

- `scripts/13b_download_lamda240_windows.ps1`
  - 纯 Windows 原生 HTTP 下载 `lamda240`，用于 WSL 网络/代理异常场景
- `scripts/13c_import_lamda240_from_windows.sh`
  - 把 Windows 下载好的 APK 导入 WSL 的 `data_lamda_eval/apks/`

**云端 train-only 侧**：

- `scripts/cloud_lamda_train_push.sh`
  - 只上传训练需要的：
    - `assets/`
    - `configs/lamda*.csv`
    - `metadata.csv`
    ...
    - `data_lamda_eval/fcg/`
- `scripts/cloud_lamda_train_only.sh`
  - 云端只训练：
    - `Phase 4 A+B+E`
    - 导出 `train / val / test / full240` embeddings
    - `Phase 6 relay + package`
    - 导出 `train / val / test / full240` GNN embeddings
- `scripts/cloud_lamda_train_start.sh`
  - 后台启动 `train-only` 流水线
- `scripts/cloud_lamda_status.sh`
  - 查看远端：
    - pid
    - 关键目录文件数
    - 训练日志尾巴

#### 4. 远端 GPU 与首轮 batch 基线（AutoDL 5090）

本轮云端设备：

- `NVIDIA GeForce RTX 5090`
- `~32 GB` 显存可见（`32607 MiB`）

首轮 `train-only` 采用的保守参数（已实际观测）：

- `Phase 4 train batch_size = 16`
- `Phase 4 export batch_size = 32`
- `Phase 4 epochs = 3`
- `Phase 6 epochs = 20`
...
- `GPU-Util ≈ 80%`
- `Memory ≈ 9415 MiB / 32607 MiB`
- `Power ≈ 443W / 575W`

**解释**：

- 说明训练已正常吃到 GPU，不是空转
- 但显存只用了约 `9.4 GB`，还有较大余量
- 这意味着当前配置是**稳定但偏保守**的

#### 5. 对后续“大规模训练更 aggressive 一点”的建议

基于当前 5090 基线，下一轮 `Phase 4` 可以考虑：

- 第一档：
  - `LAMDA_P4_BATCH_SIZE = 24`
  - `LAMDA_P4_EXPORT_BATCH_SIZE = 48`
- 若仍稳定，再试：
  - `LAMDA_P4_BATCH_SIZE = 32`

当前不建议一次性跳太大，原因是：

- `A+B+E` 的 batch sampler 会引入正例结构约束
- 有效 batch 扩张不完全等价于纯推理 batch 扩张
- 先从 `16 -> 24 -> 32` 逐步试更稳

**保守结论**：

- `16 / 32`：已验证可跑
- `24 / 48`：是下一轮最值得优先尝试的 aggressive 设定

#### 6. 当前 lamda240 主线（正式）

➡️ **本地**：完成 `download / disassemble / methods / SuSi / overrides / FCG`  
➡️ **云端**：只跑 `A+B+E` 与 `relay + package prior`

也就是：

1. 本地准备 `data_lamda_eval/methods/`
2. 本地准备 `data_lamda_eval/overrides/`
3. 本地准备 `data_lamda_eval/fcg/`
4. 上传到云端
5. 云端训练 `Phase 4 A+B+E`
6. 导出 `train / val / test / full240` 的 finetuned embeddings
7. 云端训练 `Phase 6 relay + package`
8. 导出 `train / val / test / full240` 的 GNN embeddings

#### 7. 当前建议

- 本轮先保持当前 `train-only` 流程跑通并拉回结果
- 等本轮结束后，再基于 5090 实测把 `Phase 4 batch` 往上提
- 不再推荐回到“远端一条龙下载+预处理+训练”的旧路线

---

**文档版本**：v1.8
**最后更新**：2026-05-14（会话 13：lamda240 本地预处理完成 + 云端 train-only 路线落地）
**下次更新触发条件**：lamda240 首轮 `Phase 4 / Phase 6` 训练完成并回收结果 / 5090 aggressive batch 复测完成
