...
### 新项目 `new_androserum`（`/usr/local/python_projects/new_androserum/`，截至 2026-05-07）
...
- 2026-05-07 起在 `43635fb` 之后新推了 **5 个 commits**（自旧到新）：
...
| 串跑 | `scripts/run_all.py`（`fire`，每步可 `do_*=false` 跳过） | 上述全集 |
...
**上云（AutoDL 等）**：`bash scripts/cloud_setup.sh`（Java 11 + conda env `androserum` + `pip install -e ".[dev]"` + GPU 自检清单）；大文件仍靠本机 `rsync assets/`；README 有逐步说明。

**测试**：`tests/test_schema.py`、`test_method_extractor.py`、`test_method_parquet.py`、`test_susi_index.py`；全量含大 ckpt：`pytest tests/test_encoder_load.py`。

**进行中 / 下一窗口**：

- Phase 3 对 `sha_dev_200` 可在本机长跑；各步 `skip_existing`，中断可续跑。
- **下一工程块**：Phase 4 — 对比学习（A+B[+E]）、损失与采样器、`scripts/04_*.py`、与 `methods/*.parquet` + baseline 向量对齐。
- Phase 5–7：未启动。
...
**2026-05-07**：仓库 `main` 已含 Phase 0–3 可运行代码与脚本（见 §5「新项目 new_androserum」与 §11 会话 2）。本节 **§7.1–7.4** 描述的是「从零拷 DexBERT 建骨架」的**历史步骤**，仅作溯源；**接手开发请以 §11 最新一条为起点，下一实作任务是 Phase 4（对比微调）**。
...
### 任务 7.5（当前）：Phase 4 — 对比学习微调

Phase 0–3 已有 `scripts/0X_*.py` 与 `run_all.py`。下一里程碑是实现 **训练循环**（损失、投影头、可选分层冻结 §8.6）、`scripts/04_*.py`（或 `src/androserum/train/` 模块 + 薄 CLI），并约定 finetuned 权重与 per-APK 向量落盘路径（例如 `data/checkpoints/`、`data/embeddings/finetuned/`）。
...
**文档版本**：v1.2
**最后更新**：2026-05-07（会话 2：Phase 0–3 代码与 handoff §5 / §7 / §11 同步）
**下次更新触发条件**：Phase 3 全量 dev200 完成 / Phase 4 首跑通 / 重大决策变更
